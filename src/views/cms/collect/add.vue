      ...
      <el-tabs v-model="activeName" class="demo-tabs">
        <el-tab-pane label="列表测试" name="list">
          ...
          >
            <div class="flex w-full">
              ...
              ></el-input>
              <el-button class="ml-5" type="primary" @click="handlePages">
                测试
              </el-button>
            </div>
          </el-form-item>
        ...
        <el-tab-pane label="内容测试" name="article">
          ...
          >
            ...
            "
              ></el-input>
              <el-button class="ml-5" type="primary" @click="handleArticle">
                测试
              </el-button>
            </div>
          </el-form-item>
        ...
        <el-tab-pane label="保存配置" name="save">
          ...
          <el-form-item>
            <el-button type="primary" v-permission-disabled="['editor','admin','super']"  @click="submit(formRef)">保存</el-button>
          </el-form-item>
        </el-tab-pane>
      </el-tabs>
    </el-form>
...
// import { ElMessage, ElForm, ElFormItem, ElInput, ElButton, ElTabs, ElTabPane, ElRadioGroup, ElRadio, ElCascader, ElCard } from 'element-plus';
...
const handlePages = async () => {
  try {
    ...
    if (res.code === 200) {
      ...
      const fullUrl = matches ? matches[1] : '';

      if (!data[0].includes('http')) {
        data = data.map(item => `${fullUrl}${item}`);
      }
      params.pages = data;
...
const handleArticle = async () => {
  try {
    ...
    if (res.code === 200) {
      ...
      article.article = res.data.article;
      // eslint-disable-next-line no-undef
      ElMessage({
        message: '内容获取成功^_^',
        type: 'success',
...
const handlecreate = async () => {
  try {
    ...
    let res = await create(paramsCopy);
    if (res.code === 200) {
      // eslint-disable-next-line no-undef
      ElMessage({
        message: '新增成功^_^',
