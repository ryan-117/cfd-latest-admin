              ...
              >
                ...
                <div class="cover flex align-center" v-if="item?.values?.length > 0">
                  <div
                    ...
                    v-for="(item, index) of item.values" :key="item.url"
                  >
  ...
  try {
    ...
    if (res.code === 200) {
      viewsList.value = res.data
        .filter(item => item !== '404.html' && item !== '500.html')
...
const searchTag = async (keywords = '') => {
  try {
    let res = await search(cur.value, keywords, 100)
    if (res.code === 200) {
  ...
  try {
    ...
    if (res.code === 200) {
      res.data.fields.forEach(item => {
        if (
          item.defaultVal &&
          item.defaultVal.includes('[{') &&
          item.defaultVal.includes('options')
        ) {
          let s = JSON.parse(item.defaultVal)
          item.defaultVal = s.options || []
        }
        if (item.type == '8') {
          item.values = []
        }
      ...
      field.value.forEach((item) => {
        if (fieldParams?.value?.[item.ename]?.includes('[')) {
          item.values = JSON.parse(fieldParams.value[item.ename])
        } else {
          item.values = fieldParams?.value?.[item.ename]
        }
      })
...
const getDetail = async () => {
  try {
    ...
    if (res.code === 200) {
      let p = res.data
     // p.content = htmlDecode(p.content)
      p.attr = p.attr ? p.attr.split(',') : []
      p.subCid = p.subCid ? p.subCid.split(',').map(Number) : []
      p.oldTagId = p.tagId
      p.tagId = p.tagId ? p.tagId.split(',').map(Number) : []
      p.status = p.status.toString()
      p.updatedAt = new Date(p.createdAt)
      Object.assign(params, p)
      fieldParams.value = p.field || {}
      loading.value = false
    }
  } catch (error) {
    console.error(error)
...
const articleUpdate = async () => {
  try {
    ...
    paramsCopy.subCid = paramsCopy.subCid.toString()
    paramsCopy.tagId = paramsCopy.tagId.toString()

    // 判断是否有缩略图
    if (!paramsCopy.img && autoImg.value) {
      paramsCopy.img = getImgUrlFromStr(paramsCopy.content)[0]
    }

    if (!paramsCopy.description && autoDes.value) {
      paramsCopy.description = filterHtml(paramsCopy.content).substr(0, 255)
    }

    field.value.map((item) => {
      fieldParams.value[item.ename] =
        typeof item.values === 'object'
          ? JSON.stringify(item.values)
          : item.values
    })

    let res = await update({ ...paramsCopy, field: fieldParams.value })
    if (res.code === 200) {
      // eslint-disable-next-line no-undef
      ElMessage({
        message: '更新成功^_^',
